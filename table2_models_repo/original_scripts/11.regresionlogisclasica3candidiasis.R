# ============================================================
# REGRESIÓN LOGÍSTICA MULTIVARIADA – Candidiasis invasiva
# Lee el archivo 11.regresionlogisclasica3.xlsx
# Columna 1  = CANDIDEMIA (Candidiasis invasiva / Candidiasis no invasiva)
...
# ---------- 1) Rutas y lectura ----------
dir_base  <- "C:/Users/pc/Desktop/EXTRACCION INFORMACION REXCEL/candidiasis"
...
df <- df_raw |>
  mutate(
    ...
    !!col_dep := case_when(
      !!sym(col_dep) %in% c("Candidiasis invasiva", "candidiasis invasiva", "1") ~ 1,
      !!sym(col_dep) %in% c("Candidiasis no invasiva", "candidiasis no invasiva", "0") ~ 0,
      TRUE ~ NA_real_
    )
  ) |>
...
salida_xlsx <- file.path(
  dir_base,
  "11_REGRESION_LOGISTICA_CANDIDIASIS_3V_RESULTADOS.xlsx"
)
