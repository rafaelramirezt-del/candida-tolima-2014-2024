# ============================================================
# TABLE 2 – Model 2: Invasive candidiasis (standard logistic regression)
# Input : data/11.regresionlogisclasica3candidiasis.xlsx  (or similar)
# Output: outputs/model_invasive_candidiasis_glm.xlsx
# ============================================================
...
# ---- Input discovery ----
in_default <- file.path(data_dir, "11.regresionlogisclasica3candidiasis.xlsx")
if (!file.exists(in_default)) {
  cand <- list.files(data_dir, pattern = "candidiasis.*\\.xlsx$", ignore.case = TRUE, full.names = TRUE)
  # avoid picking the no-albicans file by accident
  cand <- cand[!grepl("albican", cand, ignore.case = TRUE)]
  if (length(cand) == 0) stop("Input file not found in /data (expected *candidiasis*.xlsx).")
  in_default <- cand[1]
}

out_xlsx <- file.path(out_dir, "model_invasive_candidiasis_glm.xlsx")
...
df <- df_raw |>
  mutate(
    !!col_dep := case_when(
      !!sym(col_dep) %in% c("Candidiasis invasiva","candidiasis invasiva","1",1) ~ 1,
      !!sym(col_dep) %in% c("Candidiasis no invasiva","candidiasis no invasiva","0",0) ~ 0,
      TRUE ~ NA_real_
    )
  ) |>
